...
for dataset, template in out_loop:
    ...
    for sim_strat, sampling_strat in inner_loop:
        ...
        for act1, act2 in prods:
            ...
            scores_ = dict()
            s_log = sim_log[(sim_log.activity_1==act1) & (sim_log.activity_2==act2)].copy()
            if s_log.empty:
                continue 
            ...
            if sampling_strat == "argmax":
                ...
                sv = []
                for i in s_log.sim_id.unique():
                    variants = s_log[s_log.sim_id==i].groupby("case_id").activity.apply(list).values
            ...
            if template == "existence":
                ...
                l = []
                for ix in s_log.sim_id.unique():
                    sim_rate = positive_relations(s_log[s_log.sim_id == ix], act1, act2)
                    l.append(sim_rate)
            ...
            for df in dfs:
                ...
                print(clf_report["weighted avg"]["precision"])
            ...
            scores_["accuracy"] =                   clf_report["accuracy"]
